...
flags.DEFINE_string('weights', './checkpoints/yolov4-5-5-2021',
...
def main(_argv):
    ...
    for count, image_path in enumerate(images, 1):
        ...
        time_string = time.strftime("%d_%m_%Y-%H_%M_%S", named_tuple)
        tele_time = time.strftime("%d/%m/%Y lúc %H:%M:%S", named_tuple)
        cv2.imwrite(FLAGS.output + 'Image' + '_' + str(time_string) + '.png', image)
        files = {'photo': open(FLAGS.output + 'Image' + '_' + str(time_string) + '.png','rb')}
        resp = requests.post('https://api.telegram.org/bot1712983599:AAEKYKHcV_adSc_UrnAEx-9ABnNJKhomwgA/sendPhoto?chat_id=-409481190&caption=Có {} đối tượng được phát hiện vào {} !!'.format(len(counted_classes), str(tele_time)), files=files)
