...
flags.DEFINE_string('weights', './checkpoints/yolov4-17May',
                    'path to weights file')
flags.DEFINE_integer('size', 416, 'resize images to')
flags.DEFINE_boolean('tiny', False, 'yolo or yolo-tiny')
flags.DEFINE_string('model', 'yolov4', 'yolov3 or yolov4')
flags.DEFINE_string('output', './detections/output{}.avi'.format(time_string), 'path to output video')
...
def main(_argv):
    ...
    # cho camera
    # vid = cv2.VideoCapture('rtsp://admin:D9ng2612@192.168.1.180:554/cam/realmonitor?channel=1&subtype=1')
    ...
    while True:
        ...
        crop_path = os.path.join(os.getcwd(), 'detections', 'crop', 'real-time2')
        try:
            os.mkdir(crop_path)
        except FileExistsError:
            pass
        if frame_num % crop_rate == 0:
            crop_objects(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), pred_bbox, crop_path, allowed_classes)
        else:
            pass
